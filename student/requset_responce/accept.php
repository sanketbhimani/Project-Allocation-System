...
mysql_query($q)
		...
		for($i=1;$i<=6;$i++){
				...
				$q = "UPDATE `studentdata` SET `req".$i."` = '' WHERE `req".$i."` = '".$studentid."'";  //and studentid = mara account nu studentid
				//echo($q.'<br>');
					mysql_query($q)
		or die("sql query2".mysql_error());
						$q = "UPDATE `studentdata` SET `sreq".$i."` = '' WHERE `sreq".$i."` = '".$mystudentid."'"; // and studentid = jeni request aivi enu studentid
							mysql_query($q)
		or die("sql query2".mysql_error());
...
$cnt++;
$q = "UPDATE `teamdata` SET `mem".$cnt."` = '".$mystudentid."' WHERE `tmid` = '".$tmid."'";
mysql_query($q);	
		mysql_query("UPDATE `studentdata` SET `tmid` = '".$tmid."' WHERE `studentid` = '".$mystudentid."'");
